...
function mame-check {
  ...
  if [ ! -z $MAMEVER ]; then
    ...
    if [ -z $LATESTMAMEVER ]; then echo $MAMEVER ERROR; exit; fi 
...
function hypseus-check {
  ...
  if [ ! -z $HYPSEUSVER ]; then
    ...
    if [ -z $LATESTHYPSEUSVER ]; then echo $HYPSEUSVER ERROR; exit; fi  

    LATESTHYPSEUSVER=${LATESTHYPSEUSVER##v}             # Strip the leading v
    HYPSEUSVER=$(echo $HYPSEUSVER | sed 's/^v*//' | sed 's/-RPi$//')    
